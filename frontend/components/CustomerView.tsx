...
export default function CustomerView() {
  ...
  return (
    <div className="space-y-4"><div className="bg-white rounded-2xl p-4 shadow-sm space-y-3">
      ...
      {approveTx && !approveOk && <TxPending hash={approveTx} label="Approving cUSD spend..." />}
      {payTx && !payOk && <TxPending hash={payTx} label="Processing payment..." />}
      {amount&&Number(amount)<0.01&&<p className="text-xs text-red-500">Minimum payment is 0.01 cUSD.</p>}
      <button onClick={handlePay} disabled={!merchantId||!amount||Number(amount)<0.01||!contractAddress||!!approveTx||!!payTx} className="w-full py-3 bg-amber-600 text-white rounded-xl font-semibold disabled:opacity-50 active:scale-95 transition-transform">Pay Now</button>
    </div></div>
  );
}
